...
- [ ] 1.6 Calculate baseline for primary user: `12345.60 - (income_since_jan15 - expenses_since_jan15)`
- [ ] 1.7 Mark all income entries before Jan 15 with `exclude_from_calculations = true`
...
- [ ] 6.1 Get user's tracking_start_date from users table
- [ ] 6.2 Filter all income queries: `WHERE date >= tracking_start_date AND exclude_from_calculations = false`
- [ ] 6.3 Filter all expense queries: `WHERE expense_date >= tracking_start_date`
- [ ] 6.4 Update emergency fund calculation to use tracking period only

## 7. Backend - Current Total Money Endpoint

- [ ] 7.1 Create `GetCurrentTotalMoney` handler in handler_stats.go
- [ ] 7.2 Fetch user's money_baseline and tracking_start_date
- [ ] 7.3 Calculate total income since tracking_start_date (exclude_from_calculations = false)
- [ ] 7.4 Calculate total expenses since tracking_start_date
- [ ] 7.5 Return: `current_total = baseline + (income - expenses)`, include breakdown in response
- [ ] 7.6 Add route `GET /api/budget/current-total-money` in routes.go
- [ ] 7.7 Add Swagger annotations

## 8. Backend - User Settings Endpoints (Future Enhancement)
...
- [ ] 9.1 Run `go build ./...` and fix compilation errors
- [ ] 9.2 Run `go vet ./...`
...
- [ ] 9.5 Regenerate Swagger docs with `make swagger`

## 10. Frontend - Types & API Client

- [ ] 10.1 Add `CurrentTotalMoneyResponse` type (current_total, baseline, net_change, income_since, expenses_since, tracking_start_date)
- [ ] 10.2 Add `fetchCurrentTotalMoney()` API function in lib/api.ts
- [ ] 10.3 Update `SavingsRateResponse` to include tracking_period_start (optional for now)
- [ ] 10.4 Update `FiftyThirtyTwentyResponse` to include tracking_period_start

## 11. Frontend - React Query Hooks

- [ ] 11.1 Add `useCurrentTotalMoney()` hook in use-budget.ts
- [ ] 11.2 Add query key: `budgetKeys.currentTotalMoney()`

## 12. Frontend - Current Total Money Card

- [ ] 12.1 Create `CurrentTotalMoneyCard` component in components/budget/
- [ ] 12.2 Display current total in large text with ₱ symbol
- [ ] 12.3 Show breakdown: baseline, income since tracking, expenses since tracking, net change
- [ ] 12.4 Show tracking start date: "Tracking since Jan 15, 2026"
- [ ] 12.5 Add loading and error states
- [ ] 12.6 Export from components/budget/index.ts

## 13. Frontend - Dashboard Integration

- [ ] 13.1 Add CurrentTotalMoneyCard to dashboard page (top row, prominent position)
- [ ] 13.2 Verify it displays correct data from API
- [ ] 13.3 Add tooltip explaining baseline concept

## 14. Frontend - Income List Visual Indicator

- [ ] 14.1 Update income list component to check `exclude_from_calculations` field
- [ ] 14.2 Add grey badge "[Historical]" next to excluded entries
- [ ] 14.3 Add tooltip: "Excluded from budget calculations (before tracking start date)"

## 15. Frontend - Lint & Verify

- [ ] 15.1 Run `npx tsc --noEmit` and fix TypeScript errors
- [ ] 15.2 Run `pnpm lint` and fix ESLint errors in modified files only
