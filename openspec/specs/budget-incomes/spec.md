...
Users SHALL create recurring daily income rules that auto-calculate daily amounts from a start date.

#### Scenario: Create recurring daily income
- **WHEN** user creates income entry with recurring rule set to daily
- **THEN** system stores amount, start_date, description, and user_id
...
- **THEN** system includes amount × number of days from max(S, X) to Y
