...
export async function fetchCategoryBreakdown(startDate?: string, endDate?: string): Promise<CategoryBreakdown[]> {
  ...
  const queryString = params.toString();
  const endpoint = queryString ? `/api/budget/stats/category-breakdown?${queryString}` : "/api/budget/stats/category-breakdown";
...
export async function fetchTrends(granularity: "day" | "month" = "month", startDate?: string, endDate?: string): Promise<TrendItem[]> {
  const params = new URLSearchParams();
