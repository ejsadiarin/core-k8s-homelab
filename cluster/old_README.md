...
export SETUP_NODEIP=192.168.10.171  # Your node IP
export SETUP_CLUSTERTOKEN=randomstring1234567  # Strong token
...
# On each WORKER node:
export MASTER_IP=192.168.10.171  # IP of your master node
...
<!-- TODO: watch the youtube video for this for the gotchas: https://www.youtube.com/watch?v=AY5mC5rDUcw -->
...
export TUNNEL_NAME="k3s-homelab-tunnel"  # Must match config.yaml
```
...
export DOMAIN="yourdomain.com"
export TUNNEL_NAME="k3s-homelab-tunnel"  # This should match the name in your config.yaml
...
# Deploy applications
kubectl apply -f apps/myapplications-appset.yaml
```
...
2. **Deployment Strategy Changes for RWO Volume Apps:**
   ...
   # apps/homepage-dashboard/deployment.yaml
   spec:
     strategy:
       type: Recreate  # Added for RWO volume safety
   
   # apps/proxitok/deployment.yaml  
   spec:
     strategy:
       type: Recreate  # Added for cache PVC
   
   # apps/searxng/redis.yaml
   spec:
     strategy:
